...
// Get data and information by user
const findContctIdByUser = async (req, res) => {
    const id = req.params.id;
    const findId = await contactModel.findOne({user:id});
    if (!findId) {
        return res.status(400).json({ success: false, message: "Id does not found" });
    }
    res.status(200).json({ success: true, message: "All Contact has been fetched by user", findId});
}
...
module.exports = { contactRequest, getContact, findContctIdByUser, findContactId, updateContactById, deleteDataById };